...
def go_enrichment(
    ...
    if "Odds Ratio" in df.columns:
        ...
        df["OddsRatio"] = df["OddsRatio"].replace([np.inf, -np.inf], np.nan)
    else:
        df["OddsRatio"] = np.nan
    ...
    if "Combined Score" in df.columns:
        ...
        df["CombinedScore"] = df["CombinedScore"].replace([np.inf, -np.inf], np.nan)
...
def plot_go_terms(
    ...
    cap_quantile: float = 0.75,
    cap_threshold: float = 30.0,
) -> plt.Figure:
    ...
    Args:
        ...
        cap_quantile (float): Quantile of |xaxis| values used as the axis
            cap candidate when one term dominates the scale. Default 0.75.
        cap_threshold (float): Minimum value the quantile-based cap must
            reach before capping is applied at all. Prevents small-scale
            metrics (e.g. GeneRatio, max 1.0) from being "capped" at a
            tiny number. Default 30.

    Returns:
        plt.Figure: The rendered matplotlib figure.

    Raises:
        ValueError: If xaxis is not one of the accepted options, or
            cap_quantile is not in (0, 1].
    ...
    # ── axis capping ─────────────────────────────────────────────────────────
    # If one bar (e.g. OR=80) massively outscales the rest (~30), it
    # squashes every other bar toward zero. Cap the axis at a quantile
    # of the current values instead of the raw max — but only if that
    # quantile actually clears cap_threshold, so small-range metrics
    # (GeneRatio 0-1, modest CombinedScores) are never touched.
    abs_vals = df["plot_val"].abs()
    raw_max = abs_vals.max()
    quantile_cap = abs_vals.quantile(cap_quantile)

    apply_cap = (quantile_cap >= cap_threshold) and (raw_max > quantile_cap)
    axis_cap = quantile_cap if apply_cap else raw_max
    if axis_cap == 0:
        axis_cap = 1  # degenerate all-zero case guard

    df["plot_val_capped"] = df["plot_val"].clip(lower=-axis_cap, upper=axis_cap)
    df["is_truncated"] = df["plot_val"].abs() > axis_cap
    ...
    bars = ax.barh(df["label"], df["plot_val_capped"], color=colors, edgecolor="none")

    # gene count labels at bar tips (based on capped axis scale, not raw max)
    offset = axis_cap * 0.01
    ...
    for bar, (_, row) in zip(bars, df.iterrows()):
        ...
        ax.annotate(
            ...
            xytext=(4 if x >= 0 else -4, 0),   # 4 points of real screen gap
            textcoords="offset points",
            va="center", ha=ha, fontsize=12,
        )
        # truncation marker: bar was clipped, show real value was higher
        if row["is_truncated"]:
            arrow = "\u25B6" if x >= 0 else "\u25C0"  # points back toward bar interior
            ax.annotate(
                f"{row[xaxis]:.0f} {arrow}",
                xy=(x, bar.get_y() + bar.get_height() / 2),
                xytext=(-6 if x >= 0 else 6, 0),   # pull inward from the tip
                textcoords="offset points",
    ...
    xlabel = xlabel_map[xaxis] + (" (capped)" if apply_cap else "")
    ...
    # symmetric x-axis: left max == right max == axis_cap (+ margin for labels)
    ax.set_xlim(-axis_cap * 1.15, axis_cap * 1.15)
...
def go_analysis(
    ...
    Pure computation step — no plotting. Pass the returned tables to
    go_plot() to render figures.

    Args:
        setting (str): Label used in console logs.
        ...
        organism (str): "human" or "mouse".
        background (list[str] | None): Background gene universe.
            None → Enrichr default background.
        pvalue_cutoff (float): Adjusted p-value threshold.
        top (int): Top N terms per direction per ontology.
    ...
    Returns:
        dict[str, pd.DataFrame]: Ontology key mapped to the combined GO
            dataframe (empty DataFrame if no terms were found for that
            ontology).

    Raises:
        ValueError: If neither de_up nor de_down is provided,
                    or if sort_by is not a valid option.
    ...
    Example:
        ...
        ... )
        >>> figs = go_plot(tables, setting="Fibroblast_vs_Epithelial")
    """
    ...
    for o in ont:
        ...
        print(f"  pvalue cutoff: {pvalue_cutoff}")
        print("  " + "─" * 44)
        ...
        if go_combine.empty:
            print(f"  No valid GO terms for {ONTOLOGY[o]}.")
...
def go_plot(
    ...
    """Render GO enrichment plots from tables produced by go_analysis().

    Args:
        tables (dict[str, pd.DataFrame]): Output of go_analysis() — ontology
            key mapped to combined GO dataframe.
        setting (str): Title and filename prefix.
    ...
    Returns:
        dict[str, plt.Figure | None]: Ontology key mapped to Figure,
            or None if that ontology had no terms.

    Raises:
        ValueError: If xaxis is not a valid option.

    Example:
        >>> tables = go_analysis(setting="cond_A", de_up=up, de_down=down)
        >>> figs = go_plot(tables, setting="cond_A", xaxis="OddsRatio")
        >>> figs = go_plot(tables, setting="cond_A", xaxis="CombinedScore",
        ...                fig_dir="figures/alt_view/")  # re-plot, no re-fetch
    """
    if xaxis not in _XAXIS_OPTIONS:
        raise ValueError(f"xaxis must be one of {_XAXIS_OPTIONS}, got '{xaxis}'.")

    figures: dict[str, plt.Figure | None] = {}

    for o, go_combine in tables.items():
        if o not in ONTOLOGY:
            warnings.warn(f"Unknown ontology '{o}', skipping.")
            continue

        if go_combine is None or go_combine.empty:
